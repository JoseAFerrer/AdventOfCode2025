        ...
        IComparisonOperators<Point2D, Point2D, bool>
{
    ...
    /// <param name="quadrant">The area to test occupancy with.</param>
    /// <returns>If the other point is in the given quadrant in relation to this point. Returns false for all numbers on the same X/Y line as the existing point.</returns>
    public readonly bool IsInQuadrant(Point2D other, Quadrant quadrant)
    {
        var result = RelativeQuadrant(other);
        if (result == Quadrant.OnGridLine) return false;
        return result == quadrant;
    }

    public readonly Quadrant RelativeQuadrant(Point2D other)
    {
